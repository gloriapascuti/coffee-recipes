...
def train_models(X_train, y_train, X_val, y_val):
    ...
    for name, model in models.items():
        ...
        macc = matthews_corrcoef(y_val, y_pred)

        results[name] = {
            'model': model, 'accuracy': accuracy, 'precision': precision,
            'recall': recall, 'f1': f1, 'roc_auc': roc_auc, 'macc': macc,
    ...
    macc_ensemble = matthews_corrcoef(y_val, y_pred_ensemble)
    
    results['Ensemble (RF + GB)'] = {
        'model': ensemble, 'accuracy': accuracy_ensemble, 'precision': precision_ensemble,
        'recall': recall_ensemble, 'f1': f1_ensemble, 'roc_auc': roc_auc_ensemble, 'macc': macc_ensemble,
...
def evaluate_model_detailed(best_model, best_model_name, results, y_val):
    ...
    print(f"  Macc:    {best_results['macc']:.3f}")
...
def main():
    ...
    print(f"MCC: {results[best_model_name]['macc']:.3f}")
